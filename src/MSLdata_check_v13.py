...
def process_attendance_data(input_file: str, start_from_saturday: bool):
    ...
    progress(f"openpyxlで読み込み中: {input_file}")
    import openpyxl
    ...
    for row in data_rows:
        ...
        if str(r_dict.get("出欠", "")).strip() != "出席": continue

        cls = str(r_dict.get("教室", "")).strip()
        cls = re.sub(r"(ＦＳ|FS|ｆｓ|fs)", "", cls).strip()

        name = str(r_dict.get("担当講師名", "")).strip()
        name = re.sub(r"[Ａ-Ｚ_]", "", name).strip()

        no = str(r_dict.get("担当講師N0", "")).strip()

        dt_val = r_dict.get(date_col) if date_col else None
        week_num = None
        dt_obj = None
        ...
        for n_c, s_c, e_c in hw_cols_groups:
            hw_name = str(r_dict.get(n_c, "")).strip()
            ...
            try:
                s_m = re.search(r"(\d+)", str(r_dict.get(s_c, "")))
                e_m = re.search(r"(\d+)", str(r_dict.get(e_c, "")))
                if s_m and e_m:
        ...
        has_lap = 1 if any(str(r_dict.get(c, "")).strip() and str(r_dict.get(c, "")).strip().lower() != "none" for c in lap_col_names) else 0
        has_kaku = 1 if any(str(r_dict.get(c, "")).strip() and str(r_dict.get(c, "")).strip().lower() != "none" for c in kaku_col_names) else 0
        has_test = 1 if (has_lap or has_kaku) else 0

        grade = str(r_dict.get("学年", ""))
        subj = str(r_dict.get("科目", ""))
        excl_grade = 1 if re.search(r"(高3|高３|高卒)", grade) else 0
        excl_subj = 1 if re.search(r"(国語|小論文)", subj) else 0
