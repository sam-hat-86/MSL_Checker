...
def format_excel_fast(all_pivot_df: pl.DataFrame, excluded_pivot_df: pl.DataFrame, classroom_summary_df: pl.DataFrame, excluded_classroom_summary_df: pl.DataFrame, output_file: str, date_range_str: str, sheet_name: str, week_period_labels_all: list[str], week_period_labels_excl: list[str]):
    ...
    def write_sheet_v7(sheet_df: pl.DataFrame, summary_df: pl.DataFrame, week_period_labels: list[str], sheet_title: str):
        ...
        for h in headers:
            ...
            if m:  # マッチした場合のみ処理することで、PylanceのOptional警告を解消
        ...
        for col_idx, col_name in enumerate(headers):
            str_lens = [len(str(v)) for v in col_values.get(col_name, [])]
            max_len = max(max(str_lens) if str_lens else 0, len(col_name))
...
# 印刷・改ページネイティブ高速処理 (openpyxlの代替)
    ...
    write_sheet_v7(all_pivot_df, classroom_summary_df, week_period_labels_all, "全集計")
    write_sheet_v7(excluded_pivot_df, excluded_classroom_summary_df, week_period_labels_excl, "除外集計")
...
def main():
    ...
    try:
        ...
        (all_pivot_df, excluded_pivot_df, classroom_summary_df, excluded_classroom_summary_df, output_file, date_range_str, sheet_name, week_period_labels_all, week_period_labels_excl) = process_attendance_data(input_file)
        format_excel_fast(all_pivot_df, excluded_pivot_df, classroom_summary_df, excluded_classroom_summary_df, output_file, date_range_str, sheet_name, week_period_labels_all, week_period_labels_excl)
