...
def process_attendance_data(input_file: str, start_from_saturday: bool) -> tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame, pl.DataFrame, str, str, str, list[str], list[str], dict[str, int], dict[str, int], dict[str, str]]:
    ...
    if valid_dates.height > 0:
        ...
        date_range_str = f"期間:{min_dt.strftime('%Y/%m/%d')}({get_jp_weekday(min_dt)}) ～ {max_dt.strftime('%Y/%m/%d')}({get_jp_weekday(max_dt)})"
    else:
    ...
    def aggregate_and_pivot(metrics_df: pl.DataFrame):
        ...
        for i, w in enumerate(week_nums, start=1):
            ...
            rename_map[f"授業数_{w}"], rename_map[f"宿題平均_{w}"], rename_map[f"ラップ回数_{w}"] = f"授業_{i}", f"平均HW_{i}", f"Lap実施数_{i}"
            rename_map[f"ラップ率_{w}"], rename_map[f"テスト回数_{w}"], rename_map[f"テスト率_{w}"] = f"Lap％_{i}", f"テスト実施数_{i}", f"テスト％_{i}"
...
def get_column_style_config(orig_key: str, clean_name: str, max_val_len: int) -> tuple[float, str, str | None]:
    ...
    if clean_name == "総授業数": return max(max_len + 2.0, 9.0), "lavender", None
...
def format_excel_fast(
    ...
    def write_sheet_v7(sheet_df: pl.DataFrame, summary_df: pl.DataFrame, week_period_labels: list[str], sheet_title: str, max_lens_sheet: dict[str, int], tab_color: str):
        ...
        if nrows > 0:
            ...
            final_col_values[global_rename_map.get("担当講師N0", "No")].append("0")
            final_col_values[global_rename_map.get("担当講師名", "氏名")].append("AVG")
            ...
            col_values, nrows = final_col_values, len(final_col_values[global_rename_map.get("担当講師N0", "No")])
        ...
        for r_idx in range(nrows):
            is_avg = (col_values[global_rename_map.get("担当講師名", "氏名")][r_idx] == "AVG")
            current_row_idx = data_start + r_idx

            if is_avg:
                row_type = "avg"
                for col_idx, col_name in enumerate(headers):
                    val = col_values[col_name][r_idx]
                    orig_key = reverse_map.get(col_name, col_name)
